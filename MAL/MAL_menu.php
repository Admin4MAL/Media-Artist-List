...
function display_menu($artist_key, $context, $album_key = 0, $track_key = 0) {
	...
	if($context == 3) {			// Album Detail
        ...
        $file_name_csv = build_filename('-' . album_name($album_key) . '-album_detail.csv', $artist_key);
		echo "\t<li id=\"menu_item\" ><a href=\"MAL_csv_album_detail.php?file=$file_name_csv&album=$album_key\"><div id='menu_text'>CSV File</div></a></li>\n";
		
        $m3u_query = get_sql('album_track_list_m3u', $album_key);
        $file_name_m3u = build_filename('-' . album_name($album_key) . '-album_tracks.m3u', $artist_key);
		echo "\t<li id=\"menu_item\" ><a href=\"MAL_m3u_file.php?file=$file_name_m3u&query=$m3u_query\"><div id='menu_text'>M3U Playlist</div></a></li>\n";
	}
	...
	if($context == 4) {			// Artist track list
        ...
        $m3u_query = get_sql('artist_track_list_m3u', 0, $artist_key, 0, 6);
	...
	if($context == 5) {			// Tracks Composed by Artist list
		...
		echo "\t<li id=\"menu_item\"><a href=\"MAL_artist_track_list.php?a=$artist_key\"><div id='menu_text'>Artist Track List</div></a></li>\n";
        $query_csv = get_sql('artist_track_list_csv', 0, $artist_key, 0, 2);
        $file_name_csv = build_filename('_composed_list.csv', $artist_key);
		echo "\t<li id=\"menu_item\" ><a href=\"MAL_csv_file.php?file=$file_name_csv&query=$query_csv\"><div id='menu_text'>CSV File</div></a></li>\n";
		
        $m3u_query = get_sql('artist_track_list_m3u', 0, $artist_key, 0, 2);
        $file_name_m3u = build_filename('_composed_list.m3u', $artist_key);
		echo "\t<li id=\"menu_item\" ><a href=\"MAL_m3u_file.php?file=$file_name_m3u&query=$m3u_query\"><div id='menu_text'>M3U Playlist</div></a></li>\n";
	}
	...
	if($context == 7) { 		// Track artists responsible
        ...
        $file_name_csv = build_filename(track_name($track_key) . '-artists_responsible_list.csv');
		echo "\t<li id=\"menu_item\" ><a href=\"MAL_csv_file.php?file=$file_name_csv&query=$query\"><div id='menu_text'>CSV File</div></a></li>\n";
		
		$m3u_query = get_sql('artist_responsible_list_m3u', 0, 0, $track_key);
        $file_name_m3u = build_filename(track_name($track_key) . '-artists_responsible_list.m3u');
		echo "\t<li id=\"menu_item\" ><a href=\"MAL_m3u_file.php?file=$file_name_m3u&query=$m3u_query\"><div id='menu_text'>M3U Playlist</div></a></li>\n";
	}
	
	if($context == 8) { 		// Full Track List - alphabetical
		$query_csv = get_sql('full_track_list_title_csv');
        $file_name_csv = build_filename('full_track_list(alpha).csv');
		echo "\t<li id=\"menu_item\" ><a href=\"MAL_csv_file.php?file=$file_name_csv&query=$query_csv\"><div id='menu_text'>CSV File</div></a></li>\n";
	}
	echo "</ul></div>\n";
}
